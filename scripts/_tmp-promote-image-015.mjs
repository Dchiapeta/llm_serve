// Aponta os templates de imagem para a flux2-klein-4b-0.1.5.
//
//   node --env-file=.env scripts/_tmp-promote-image-015.mjs [--apply]
//
// Sem --apply é dry-run.
//
// O que a 0.1.5 traz e por que os DOIS templates vão juntos: encaixe de
// proporção em /v1/images/edits (uma foto retrato deixa de ser esticada até o
// canvas, que era o que fazia o modelo devolver outra pessoa) e o teto de
// multipart do agent em 41 MiB, acompanhando IMAGE_MAX_FILE_SIZE_MB=10 que já
// está gravado nos dois. Deixar o de produção para trás manteria exatamente o
// bug que motivou a versão.
//
...
const NOVA = "dchiapeta/diffusers-agent:flux2-klein-4b-0.1.5"
const ANTERIOR = "dchiapeta/diffusers-agent:flux2-klein-4b-0.1.3"
...
//
// Não há EXPECTED_DIGEST a comparar aqui como no script de criação: a 0.1.5
...
for (const nome of NOMES) {
  ...
  if (row.image === NOVA) {
    console.log(`  = ${nome}: já está na 0.1.5`)
...
console.log(
  ok
    ? "\nTudo gravado.\n\n" +
        ...
        "Até recriar, os pods seguem rodando a 0.1.3 — sem encaixe de proporção,\n" +
        "e com o agent recusando multipart acima de 21 MiB.\n\n" +
