// Aponta o GO-IMAGE-A40-QUALITY para a flux2-klein-4b-0.1.7.
//
//   node --env-file=.env scripts/_tmp-promote-image-017.mjs [--apply]
//
// Sem --apply é dry-run.
//
// O que a 0.1.7 traz: grade da referência igual à da saída em
// /v1/images/edits sem `size` (canvas ≤ 1 MP + foto redimensionada para ele).
// SÓ o QUALITY: o GO-IMAGE-A40 de produção não tem 816x1216 na allowlist, e com
// a 0.1.7 uma selfie retrato cairia no canvas quadrado. Promover a produção só
// depois de incluir o 816x1216 lá.
//
// DEPOIS DE APLICAR: recriar a máquina. Template atualizado não troca a
// imagem de um pod em execução.

import { createClient } from "@supabase/supabase-js"

const NOVA = "dchiapeta/diffusers-agent:flux2-klein-4b-0.1.7"
const ANTERIOR = "dchiapeta/diffusers-agent:flux2-klein-4b-0.1.6"
const NOMES = ["GO-IMAGE-A40-QUALITY"]
...
//
// Não há EXPECTED_DIGEST a comparar aqui como no script de criação: a 0.1.7
...
for (const nome of NOMES) {
  ...
  if (row.image === NOVA) {
    console.log(`  = ${nome}: já está na 0.1.7`)
...
console.log(
  ok
    ? "\nTudo gravado.\n\n" +
        "FALTA (é você quem sobe o pod): RECRIAR a máquina do GO-IMAGE-A40-QUALITY.\n" +
        "Até recriar, o pod segue rodando a 0.1.6, sem a grade casada.\n\n" +
