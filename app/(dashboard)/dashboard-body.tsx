...
// Corpo do dashboard: todas as queries + computação + gráficos. Recebe
// searchParams como promise (resolvida aqui, não no shell) para o título da
// página aparecer instantâneo. O reconcile com o RunPod fica aqui, mas o
// /health por máquina sai do caminho — streama via <LiveStatusBadge>.
export async function DashboardBody({
  searchParamsPromise,
}: {
  searchParamsPromise: Promise<{ period?: string }>
}) {
  const { period: rawPeriod } = await searchParamsPromise
