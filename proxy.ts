...
export const config = {
  matcher: [
    ...
    // de modelo por stack e a de emissão de chave "customer" — chamadas por
    // sistemas externos (LP/admin de outro projeto) com
    // X-External-Secret/EXTERNAL_INTEGRATION_SECRET. As
    ...
    "/((?!_next/static|_next/image|favicon.ico|api/agent|api/machines/provision|api/machines/[^/]+/recreate|api/knowledge/ingest|api/stacks$|api/stacks/[^/]+/billing-sync|api/stacks/[^/]+/model-config|api/keys).*)",
  ],
}
